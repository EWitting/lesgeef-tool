...
def create_rooster_config_tab():
    ...
    ui.label("Wijzigingen worden direct toegepast. Gebruik 'Opslaan' om permanent op te slaan.").classes(
    ...
    with ui.grid(columns=2).classes("w-full max-w-lg q-gutter-y-sm"):
        fields["lesgever_minimum"] = ui.number(
            "Minimum lesgevers per les", value=cfg.lesgever_minimum, min=0, max=10, step=1, on_change=on_change
        ).props("outlined dense")
        fields["lesgever_maximum"] = ui.number(
            "Maximum lesgevers per les", value=cfg.lesgever_maximum, min=1, max=10, step=1, on_change=on_change
        ).props("outlined dense")

        fields["penalty_lesgever_tekort"] = ui.number(
            "Penalty: lesgever tekort", value=cfg.penalty_lesgever_tekort, min=0, step=1, on_change=on_change
        ).props("outlined dense")
        fields["penalty_misschien"] = ui.number(
            "Penalty: Misschien", value=cfg.penalty_misschien, min=0, step=1, on_change=on_change
        ).props("outlined dense")

        fields["penalty_geen_ervaren_lesgever"] = ui.number(
            "Penalty: geen ervaren lesgever", value=cfg.penalty_geen_ervaren_lesgever, min=0, step=1, on_change=on_change
        ).props("outlined dense")
        fields["penalty_meerdere_lessen_per_week"] = ui.number(
            "Penalty: meerdere lessen/week", value=cfg.penalty_meerdere_lessen_per_week, min=0, step=1, on_change=on_change
        ).props("outlined dense")

        fields["richtlijn_lessen_per_week"] = ui.number(
            "Richtlijn lessen per week", value=cfg.richtlijn_lessen_per_week, min=0, max=5, step=0.1, format="%.2f", on_change=on_change
        ).props("outlined dense")
        fields["penalty_boven_richtlijn"] = ui.number(
            "Penalty: boven richtlijn", value=cfg.penalty_boven_richtlijn, min=0, step=1, on_change=on_change
        ).props("outlined dense")

        fields["penalty_onder_richtlijn"] = ui.number(
            "Penalty: onder richtlijn", value=cfg.penalty_onder_richtlijn, min=0, step=1, on_change=on_change
        ).props("outlined dense")
        fields["lesgever_bonus"] = ui.number(
            "Bonus: extra lesgever", value=cfg.lesgever_bonus, min=0, step=1, on_change=on_change
        ).props("outlined dense")

    status = ui.label("").classes("text-caption q-mt-sm")

    with ui.row().classes("q-mt-md q-gutter-sm"):
        ui.button("Opslaan naar bestand", icon="save", on_click=lambda: _save(status)).props("color=secondary outline")
        ui.button("Herladen van bestand", icon="refresh", on_click=lambda: _reload(fields, status)).props("flat")


def _save(status: ui.label):
    try:
